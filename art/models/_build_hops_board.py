"""Hops board: 5 varieties × 4 stages."""
import os, shutil
from PIL import Image, ImageDraw, ImageFont

RENDER_DIR = "/home/sparky/ogrs/art/models/_renders"
OUT = f"{RENDER_DIR}/HOPS_BOARD.png"
...
HOPS = [
    ("Hammerstone", "hammerstone", "#B04020 red copper"),
    ("Asgarnian",   "asgarnian",   "#D0A028 gold amber"),
    ("Yanillian",   "yanillian",   "#60A028 bright green"),
    ("Krandorian",  "krandorian",  "#603098 deep purple"),
    ("Wildblood",   "wildblood",   "#803030 dark wine red"),
]
STAGES = [
    ("1_sapling",  "Sapling"),
    ("2_growing",  "Growing"),
    ("3_partial",  "Partial Harvest"),
    ("4_mature",   "Mature (FULL)"),
]

W = LABEL_W + len(STAGES) * (CELL + PAD) + PAD
H = TITLE_H + COL_HEADER_H + len(HOPS) * (CELL + PAD) + PAD
...
draw.text((PAD, 16),
          "OGRS Hops — 5 varieties × 4 stages (trellis + climbing vine + hop cones)",
...
for ri, (label, prefix, palette) in enumerate(HOPS):
    y = TITLE_H + COL_HEADER_H + ri * (CELL + PAD)
    draw.rectangle([(0, y), (LABEL_W - PAD, y + CELL)], fill=(45, 42, 55))
    draw.text((10, y + 18), label, fill=(255, 255, 255), font=font_lbl)
    words = palette.split(" ")
    line = ""; line_y = y + 46
    for w in words:
        test = (line + " " + w).strip()
    ...
    for ci, (key, head) in enumerate(STAGES):
        ...
        src = f"{RENDER_DIR}/hops_{prefix}_{key}.png"
...
# Sync
windows_dir = "/mnt/c/OGRS_Art/models/farming/hops"
os.makedirs(windows_dir, exist_ok=True)
shutil.copy(OUT, f"{windows_dir}/HOPS_BOARD.png")
src_base = "/home/sparky/ogrs/art/models/farming/hops"
for label, prefix, palette in HOPS:
