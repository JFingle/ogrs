...
public final class mudclient implements Runnable {
	...
	// OGRS: default the client to 1.5× scale at startup. 2× was too large for
	// many monitors; 1× (the authentic 512×334) is too small. 1.5× is the
	// goldilocks default. The in-client scale dropdown (Options → Display)
	// still lets the player pick 1× / 2× / 2.5× / 3× / etc.
	public static float renderingScalar = 1.5f;
	...
	private void drawUiTabPlayerInfo(boolean var1, int var2) {
		...
		try {
			...
			if (this.uiTabPlayerInfoSubTab == 0) {
				...
				// OGRS — reverted to upstream's 2-column text list (sparky
				// 2026-05-25: "doesn't fit as well as id like... maybe just
				// the words"). The previous 3-column icon grid had low-res
				// 24×24 sprites and didn't fit the panel. Text-only is
				// authentic to RSC and reads cleanly at every UI scale.
				// Kept: green/red colouring for boosted/drained current
				// levels (modern OSRS convention, costs nothing).
				int leftColLength = Config.S_WANT_OPENPK_POINTS ? 3 : (int) Math.floor(skillCount / 2);
				int rightColLength = skillCount - leftColLength;

				// Hit-test — match upstream so the existing hover-driven
				// XP-info panel + skill-guide click both keep working.
				if (this.mouseX >= xOffset && this.mouseX <= x + 196) {
					if (this.mouseY >= heightMargin - 13 && this.mouseY <= (heightMargin + 13 * leftColLength) - 2) {
						...
						} else if (this.mouseButtonClick == 1 && this.uiTabPlayerInfoSubTab == 0) {
				...
				// Draw each skill as one line: "<Name>:@yel@<cur>/<base>"
				// Current value colouring: green if boosted above base, red
				// if drained below, white at base.
				for (currSkill = 0; currSkill < skillCount; currSkill++) {
					...
					final String curStr;
					if (cur > base)      curStr = "@gre@" + cur + "@yel@";
					else if (cur < base) curStr = "@red@" + cur + "@yel@";
					else                 curStr = "@yel@" + cur;
					this.getSurface().drawString(
						this.getSkillNames()[currSkill] + ":" + curStr + "/" + base,
						xOffset, yOffset,
						currentlyHoveredSkill == i ? textColourHovered : textColour, 1);

					yOffset += 13;
					i++;
					if (i == leftColLength) {
						if (Config.S_WANT_OPENPK_POINTS) xOffset += 10;
						xOffset += width / 2 - 10;
						yOffset = heightMargin;
					}
				}

				// After-list bookkeeping for Quest Points / Fatigue / Equipment.
				if (leftColLength != rightColLength) xOffset = x + 5;
...
//							}
	...
	private void loadSkills() {
		...
		if (S_WANT_HARVESTING)
			addSkill("Harvesting");
		// OGRS additive skills — order MUST match PayloadCustomGenerator's stat-block append order.
